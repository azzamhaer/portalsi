<script lang="ts">
  ...
  const filters = $derived([
    { v:'all',      l:$t('store.filterAll') },
    { v:'official', l:$t('store.officialFilter') },
    { v:'rating',   l:$t('store.ratingHigh') },
    { v:'sold',     l:$t('store.salesMost') }
  ]);
...
<div class="container-x py-8">
  >{$t('store.eyebrow')}<
  <h1 class="section-title mb-2">{$t('store.heading')}</h1>
  ...
  <div class="mb-8 grid gap-3 rounded-2xl border border-ink-100 bg-white p-4 lg:grid-cols-[1fr_auto]">
    <div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
      <input bind:value={search} class="input input-sm" placeholder={$t('store.searchStore')} />
      <input bind:value={city} class="input input-sm" placeholder={$t('store.city')} />
      <select bind:value={minRating} class="input input-sm">
        <option value="">{$t('store.allRating')}</option>
    ...
    <button type="button" on:click={applyFilters} class="btn-primary btn-sm">{$t('store.apply')}</button>
  </div>
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-1.5">
              ...
              {#if v.is_official}<span class="pill-ink text-[10px]">{$t('store.official')}</span>{/if}
            </div>
