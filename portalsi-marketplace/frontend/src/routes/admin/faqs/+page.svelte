<script lang="ts">
  ...
  async function save() {
    ...
    try {
      ...
      toast.success(get(t)('adm.faqSaved'));
...
{:else}
  <div class="flex items-center justify-between mb-4">
    <h3 class="font-semibold">{$t('adm.faqTitle')}</h3>
  ...
  {#if items.length === 0}
    <div class="card text-center text-ink-500 py-10">
      Belum ada FAQ. Klik <b>{$t('pf.add')}</b> untuk menambah pertanyaan baru.
    </div>
  {:else}
    <div class="space-y-3">
      {#each items as f, i}
        <div class="card">
          ...
          <div class="grid sm:grid-cols-[200px_1fr] gap-3">
            <div>
              <label class="label">{$t('adm.section')}</label>
              <input bind:value={f.section} class="input" list="faq-sections" placeholder={$t('adm.sectionPlaceholder')} />
            ...
            <div>
              <label class="label">{$t('adm.question')}</label>
              <input bind:value={f.question} class="input" placeholder={$t('adm.questionPlaceholder')} />
            </div>
          </div>
          <div class="mt-3">
            <label class="label">{$t('adm.answer')}</label>
            <textarea bind:value={f.answer} class="input" rows={3} placeholder={$t('adm.answerPlaceholder')}></textarea>
          </div>
        </div>
      {/each}
    </div>
