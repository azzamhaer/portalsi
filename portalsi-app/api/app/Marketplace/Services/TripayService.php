...
class TripayService
{
    ...
    public function __construct(
        ...
        $this->apiKey       = $apiKey       ?? \App\Marketplace\Models\Setting::get('tripay_api_key',       config('services.marketplace.tripay.api_key'));
        $this->privateKey   = $privateKey   ?? \App\Marketplace\Models\Setting::get('tripay_private_key',   config('services.marketplace.tripay.private_key'));
        $this->merchantCode = $merchantCode ?? \App\Marketplace\Models\Setting::get('tripay_merchant_code', config('services.marketplace.tripay.merchant_code'));
        $this->mode         = \App\Marketplace\Models\Setting::get('tripay_mode', config('services.marketplace.tripay.mode', 'sandbox'));
    }
        ...
        if ($this->isMockMode()) {
            return [
                ...
                'pay_url'      => in_array($method['group'], ['E-Wallet','Credit Card'])
                                ? config('services.marketplace.frontend_url', config('app.url')) . '/orders' : null,
