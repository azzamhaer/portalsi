...
use App\Marketplace\Models\Address;
use App\Marketplace\Models\Order;
use App\Marketplace\Models\Payment;
use App\Marketplace\Models\Product;
use App\Marketplace\Models\SellerVoucher;
use App\Marketplace\Models\Setting;
use App\Marketplace\Models\ShippingOption;
use App\Marketplace\Services\RajaOngkirService;
use App\Marketplace\Services\OrderPaymentService;
use App\Marketplace\Services\ProductStockNotificationService;
use App\Marketplace\Services\TripayService;
...
class OrderController extends Controller
{
        ...
        try {
            ...
            foreach ($itemsForDb as $row) {
                ...
                $trx = $this->tripay->createTransaction([
                    ...
                    'return_url'     => config('services.marketplace.frontend_url', config('app.url')) . '/orders/' . $order->id,
                ]);
    ...
    protected function sendOrderEmail($order, $user, string $event): void
    {
        ...
        try {
            \App\Marketplace\Models\UserNotification::send(
        ...
        try {
            $brevo = new \App\Marketplace\Services\BrevoService();
            $appName = \App\Marketplace\Models\Setting::get('app_name', 'MPSI');
            $frontUrl = rtrim(config('services.marketplace.frontend_url', 'http://localhost:5173'), '/');
    ...
    private function notifySellersOrderDone(Order $order): void
    {
        ...
        $vendors = \App\Marketplace\Models\Vendor::whereIn('id', $itemsByVendor->keys())->get()->keyBy('id');
    ...
    public function paymentMethods()
    {
        ...
        $db = \App\Marketplace\Models\PaymentMethod::where('is_active', true)->orderBy('group')->orderBy('sort_order')->get();
        if ($db->isNotEmpty()) return response()->json($db);
        return response()->json(\App\Marketplace\Services\TripayService::METHODS);
    }
    ...
    public function requestReturn(Request $request, $id)
    {
        ...
        if (\App\Marketplace\Models\OrderReturn::where('order_id', $order->id)->exists()) {
            return response()->json(['message' => 'Pengajuan pengembalian untuk pesanan ini hanya bisa dilakukan sekali.'], 422);
        }
        $data = $request->validate(['reason' => 'required|string|max:1000']);
        $r = \App\Marketplace\Models\OrderReturn::create([
            'order_id' => $order->id,
    ...
    private function notifyReturnRequested(Order $order, \App\Marketplace\Models\OrderReturn $return, $user): void
    {
        $admins = \App\Marketplace\Models\User::where('role', 'ADMIN')->pluck('id');
        ...
        $vendors = \App\Marketplace\Models\Vendor::whereIn('id', $vendorIds)->get();
        foreach ($vendors as $vendor) {
            if (!$vendor->user_id) continue;
            $item = $order->items->firstWhere('vendor_id', $vendor->id);
            $thread = \App\Marketplace\Models\ChatThread::firstOrCreate(
                ['user_id' => $user->id, 'vendor_id' => $vendor->id],
                ['product_id' => $item?->product_id, 'last_message_at' => null]
            );
            \App\Marketplace\Models\ChatMessage::create([
                'thread_id' => $thread->id,
