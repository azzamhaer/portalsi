...
return new class extends Migration {
    ...
    public function up(): void
    {
        ...
        DB::connection('marketplace')->table('vendors')
            ...
            ->chunkById(100, function ($vendors) {
                foreach ($vendors as $vendor) {
                    $address = DB::connection('marketplace')->table('addresses')
                    ...
                    DB::connection('marketplace')->table('vendors')->where('id', $vendor->id)->update([
        ...
        DB::connection('marketplace')->table('addresses')
        ...
        foreach ($demoVendorDefaults as $name => [$city, $province, $district, $village, $postalCode, $lat, $lng]) {
            $vendors = DB::connection('marketplace')->table('vendors')
                ->where('name', $name)
                ->where(function ($query) {
                    $query->whereNull('latitude')->orWhereNull('longitude');
                })
        ...
        DB::connection('marketplace')->table('vendors')
            ...
            ->chunkById(100, function ($vendors) use ($now) {
                foreach ($vendors as $vendor) {
                    $exists = DB::connection('marketplace')->table('addresses')
                        ->where('user_id', $vendor->user_id)
                    ...
                    $hasAddress = DB::connection('marketplace')->table('addresses')
                        ->where('user_id', $vendor->user_id)
                        ->exists();

                    $user = DB::connection('marketplace')->table('users')->where('id', $vendor->user_id)->first();

                    DB::connection('marketplace')->table('addresses')->insert([
