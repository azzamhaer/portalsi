...
return new class extends Migration {
    ...
    public function up(): void
    {
        ...
        DB::connection('marketplace')->table('vendors')->orderBy('id')->each(function ($v) {
            ...
            $candidate = $base;
            $n = 1;
            while (DB::connection('marketplace')->table('vendors')->where('username', $candidate)->where('id', '!=', $v->id)->exists()) {
                $n++;
                $suffix = '-' . $n;
                $candidate = substr($base, 0, 30 - strlen($suffix)) . $suffix;
            }
            DB::connection('marketplace')->table('vendors')->where('id', $v->id)->update(['username' => $candidate]);
        });
    }
