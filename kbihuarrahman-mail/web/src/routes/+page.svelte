<script lang="ts">
	...
	let pageTitle = $derived(
		(curFolder?.unseen ? `(${curFolder.unseen}) ` : '') + `${curLabel} — KBIHU Ar-Rahman Mail`
	);
	...
	// ── avatar sesama @kbihuarrahman.com ──
	...
	async function loadContacts() {
		...
		try {
			...
			if (r.ok) contacts = ((await r.json()) as any).contacts ?? [];
		} catch {
			/* ignore */
		}
	}
	...
	const avatarColors = ['#159c3a', '#0b8043', '#d93025', '#e37400', '#8430ce', '#0b7285', '#c2185b', '#5f6368'];
...
<div class="app" class:collapsed={!sidebarOpen}>
	...
	<aside class="sb">
		<div class="sb-head">
			...
			<span class="sb-brand"><img class="sb-logo" src="/logo.png" alt="" />KBIHU <b>Mail</b></span>
		</div>
			...
			{#if settingsTab === 'profil'}
				<div class="pp-row">
					...
					<div class="pp-info">
						...
						<span class="muted">{data.account?.email}</span>
					</div>
				</div>
...
<style>
	...
	.topbar-progress {
		...
		width: 100%;
		z-index: 200;
		background: linear-gradient(90deg, #159c3a, #8430ce, #159c3a);
	...
	.compose {
		...
		font-size: 0.95rem;
		cursor: pointer;
		box-shadow: 0 4px 14px -4px rgba(21, 156, 58, 0.7);
		margin: 2px 2px 12px;
		transition: transform 0.12s, box-shadow 0.15s;
	}
	.compose:hover {
		box-shadow: 0 6px 18px -4px rgba(21, 156, 58, 0.85);
	...
	.fab {
		...
		place-items: center;
		cursor: pointer;
		box-shadow: 0 8px 22px -4px rgba(21, 156, 58, 0.8);
	}
