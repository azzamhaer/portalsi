	...
	<div class="modal-bg" onclick={() => (settingsOpen = false)} role="presentation">
		<div class="modal wide" onclick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
			...
			{#if settingsTab === 'profil'}
				...
				<p class="note pp-note">Foto profil hanya terlihat oleh sesama pengguna <b>@sekolahimpian.com</b> (mis. saat kalian berkirim email). Pengguna email lain seperti Gmail tidak melihatnya.</p>
				...
				<form method="POST" action="?/updateProfile" use:enhance={() => { savingProfile = true; return async ({ update }) => { savingProfile = false; await update({ reset: false }); }; }}>
					...
					<p class="note">Nama ini juga dipakai sebagai nama pengirim pada email keluar.</p>
