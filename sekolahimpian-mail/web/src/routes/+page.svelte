<script lang="ts">
	...
	let pageTitle = $derived(
		(curFolder?.unseen ? `(${curFolder.unseen}) ` : '') + `${curLabel} — Sekolah Impian Mail`
	);
	...
	// ── avatar sesama @sekolahimpian.com ──
	...
	async function ensureAvatars(addrs: (string | undefined | null)[]) {
		...
		need.forEach((a) => (pending[a] = null)); // tandai agar tak difetch ulang
		avatars = pending;
		try {
		...
		<div class="modal wide" onclick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
			...
			{#if settingsTab === 'profil'}
					...
					<div class="pp-info">
						...
						<form method="POST" action="?/uploadPhoto" enctype="multipart/form-data" use:enhance={() => { uploadingPhoto = true; return async ({ update }) => { uploadingPhoto = false; await update({ reset: false }); if (photoInput) photoInput.value = ''; }; }}>
							...
							<button type="button" class="pp-link" onclick={() => photoInput?.click()} disabled={uploadingPhoto}>
								{#if uploadingPhoto}<span class="spin dark"></span> {$t('set.uploading')}{:else}<Camera size={14} /> {$t('set.changePhoto')}{/if}
							</button>
						</form>
					</div>
				</div>
				{#if (form as any)?.photoError}<p class="err-inline">{(form as any).photoError}</p>{/if}
				<p class="note pp-note">{$t('set.photoOnlyDomain')}</p>
				...
				<form method="POST" action="?/updateProfile" use:enhance={() => { savingProfile = true; return async ({ update }) => { savingProfile = false; await update({ reset: false }); }; }}>
					<label class="ml">{$t('set.fullName')}</label>
					<input class="mi" name="full_name" bind:value={profFullName} placeholder="Nama lengkap" />
					<p class="note">{$t('set.fullNameNote')}</p>
					<label class="ml">{$t('set.username')}</label>
					...
					<p class="note">{$t('set.emailFixedSekim')}</p>

					<div class="mf"><button class="cp-send" disabled={savingProfile}>{#if savingProfile}<span class="spin"></span>{:else}{$t('set.saveProfile')}{/if}</button></div>
				</form>

				<div class="sec-div"></div>
				<h3 class="sec-h sm"><MailIcon size={16} /> {$t('set.changeRecoveryEmail')}</h3>
				<p class="note">{$t('set.emailNow')} <b>{data.user?.email || '—'}</b>. {$t('set.emailChangeHint')}</p>
				...
				{:else}
					<form method="POST" action="?/changeEmail" use:enhance={() => { sendingEmail = true; return async ({ update }) => { sendingEmail = false; await update({ reset: false }); }; }}>
						<input class="mi" type="email" name="email" bind:value={newEmail} placeholder={$t('set.newEmailPlaceholder')} autocapitalize="off" spellcheck="false" />
						{#if (form as any)?.emailError}<p class="err-inline">{(form as any).emailError}</p>{/if}
						<div class="mf"><button class="cp-send" disabled={sendingEmail}>{#if sendingEmail}<span class="spin"></span>{:else}{$t('set.sendConfirmLink')}{/if}</button></div>
					</form>
				{/if}
